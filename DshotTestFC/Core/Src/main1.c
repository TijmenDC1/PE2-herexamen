...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    for (int i = 0; i < 1000; i++) {
        update_motor_buffer(0, 0, 0); // Motor 1
        update_motor_buffer(1, 0,0); // Motor 2
        update_motor_buffer(2, 0, 0); // Motor 3
        update_motor_buffer(3, 0, 0); // Motor 4
        send_dshot();

    }
    for (int i = 0; i < 1000; i++) {
            update_motor_buffer(0, 48, 0); // Motor 1
            update_motor_buffer(1, 48,0); // Motor 2
            update_motor_buffer(2, 48, 0); // Motor 3
            update_motor_buffer(3, 48, 0); // Motor 4
            send_dshot();
            HAL_Delay(10);
        }
  ...
  /* USER CODE BEGIN WHILE */
    while (1)
    {
        ...
        // daarna stabiel blijven draaien op 1000
        //for (int i = 0; i < 1000; i++){
        //update_motor_buffer(0, 1000, 0);
        //update_motor_buffer(1, 1000, 0);
        //update_motor_buffer(2, 1000, 0);
        //update_motor_buffer(3, 1000, 0);
        //send_dshot();
        //HAL_Delay(2);
        //}
    	//update_motor_buffer(0, 800 , 0);
    	//update_motor_buffer(1, 800 , 0);
    	//update_motor_buffer(2, 800 , 0);
    	//update_motor_buffer(3, 800 , 0);


        //send_dshot();
        //HAL_Delay(5);

          // 2. Laat hem daarna even stabiel draaien op waarde 100
          //for(int wacht = 0; wacht < 1000000; wacht++)
          //{
            //  update_motor_buffer(0, 400, 0);
              //update_motor_buffer(1, 400, 0);
              //update_motor_buffer(2, 400, 0);
              //update_motor_buffer(3, 400, 0);
