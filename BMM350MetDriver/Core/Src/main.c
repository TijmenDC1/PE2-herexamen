...
struct bmm350_mag_temp_data comp_data;
struct bmm350_self_test test;
...
void stm32_delay_us(uint32_t period, void *intf_ptr) {
    // STM32 werkt in milliseconden, Bosch vraagt microseconden
...
int main(void)
{
  ...
  */
  ...
  /* USER CODE BEGIN 2 */
    ...
    // 3. Start de Bosch Initialisatie
    int returnwaarde = bmm350_init(&dev);

    if (returnwaarde == BMM350_OK) {
        printf("Init succesvol!\n");
    } else {
        printf("Init gefaald, error code: %d\n", returnwaarde);
        while(1) { HAL_Delay(100); } // Blokkeer hier als het misgaat
    }

    // 4. Instellingen toepassen
    bmm350_set_powermode(BMM350_NORMAL_MODE, &dev);
    bmm350_set_odr_performance(BMM350_DATA_RATE_50HZ, BMM350_NO_AVERAGING, &dev);
    bmm350_enable_axes(BMM350_X_EN, BMM350_Y_EN, BMM350_Z_EN, &dev);


  //BMP384_Init(&calibData);
  uint8_t bmm350_adres = (0x14 << 1); // Gaat uit van ADSEL aan GND
  uint8_t buffer[3] = {0}; // Array voor: dummy1, dummy2, echte_data
  uint8_t chip_id = 0;
  uint8_t pData;

  // We lezen nu 3 bytes vanaf register 0x00
  HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c1, bmm350_adres, 0x00, I2C_MEMADD_SIZE_8BIT, buffer, 3, 100);
  uint8_t data[3];
  	HAL_I2C_Master_Transmit(&hi2c1, (0x14 << 1), 0x00, 1, 100);
  	HAL_I2C_Master_Receive(&hi2c1, (0x14 << 1), data, 3, 100);	// we lezen de 2 dummy bytes in + de goeie byte

  	for(int i = 0; i < 1; i++) {
  		pData = data[i + 2];	// we zetten hier de goeie byte in een nieuwe var
  	}
  	chip_id = pData;
  if (status == HAL_OK) {
      chip_id = buffer[2]; // Negeer de twee 0x00 dummy bytes!

      if (chip_id == 0x33) {
    	  printf("Communicatie en id zijn juist\n");
    	            HAL_Delay(5000);
      } else {
          printf("Communicatie maar foute id\n");
          HAL_Delay(5000);
      }
  } else {
	  HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c1, bmm350_adres, 0x00, I2C_MEMADD_SIZE_8BIT, buffer, 3, 100);
	    uint8_t data[3];
	    	...
	    	chip_id = pData;
	  printf("Eerste test is fout gegaan, ID: %d\n", chip_id);
	  printf("Status is %d", status);
	  HAL_Delay(4000);
	  int returnwaarde  = bmm350_init(&dev);
	    while(returnwaarde != 0){
	  	  returnwaarde = bmm350_init(&dev);
	  	printf("Tweede test is fout gegaan, ID: %d\n", chip_id);
	  		  HAL_Delay(4000);
	    };
      // FOUT! status is geen HAL_OK. Nu is er wél iets mis met de hardware.
  }
  HAL_Delay(4000);
  printf("Init klaar \n");
  bmm350_set_powermode(BMM350_NORMAL_MODE, &dev);
  ...
  bmm350_read_uncomp_mag_temp_data(&raw_data, &dev);
  bmm350_set_int_ctrl_ibi(BMM350_IBI_ENABLE, BMM350_CLEAR_ON_IBI, &dev);
  bmm350_get_compensated_mag_xyz_temp_data(&comp_data, &dev);
  bmm350_perform_self_test(&test, &dev);
  bmm350_set_i2c_wdt(BMM350_I2C_WDT_DIS, BMM350_I2C_WDT_SEL_SHORT , &dev);
  /* USER CODE END 2 */
  ...
  {
	  bmm350_read_uncomp_mag_temp_data(&raw_data, &dev);
	  printf("Raw Data: X: %lu, Y: %lu, Z: %lu, Extra: %lu\n", raw_data.raw_xdata, raw_data.raw_ydata, raw_data.raw_zdata, raw_data.raw_data_t);
	  bmm350_get_compensated_mag_xyz_temp_data(&comp_data, &dev);
	  printf("Comp Data: X: %f, Y: %f, Z: %f, temp: %f\n", comp_data.x, comp_data.y, comp_data.z, comp_data.temperature);
	  bmm350_perform_self_test(&test, &dev);
	  printf("Comp Data: X: %f, XH: %f, XL: %f, Y: %f, YH: %f, YL: %f\n", test.out_ust_x, test.out_ust_xh, test.out_ust_xl, test.out_ust_y, test.out_ust_yh, test.out_ust_yl);
