...
/* USER CODE BEGIN PV */
static BMM350v2_Calib mag_calib;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_UART8_Init();
  /* USER CODE BEGIN 2 */
  BMM350v2_Status mag_status = BMM350v2_Init();

  if (mag_status == BMM350V2_OK) {
      /* Kalibratie: draai de sensor rustig een paar keer rond alle assen
       * terwijl dit loopt (~5s), dan bepalen we de hard-iron offset. */
  ...
  while (1)
  {
	  if (mag_status != BMM350V2_OK) {
		  ...
		  int32_t x, y, z;
		  if (BMM350v2_ReadRaw(&x, &y, &z) == BMM350V2_OK) {
			  /* geen roll/pitch beschikbaar in dit project (geen IMU hier),
			   * dus 0.0/0.0: enkel geldig als de sensor vlak ligt. */
			  float heading = BMM350v2_TiltCompensatedHeading(x, y, z, &mag_calib, 0.0f, 0.0f);
			  printf("BMM350v2: x=%ld y=%ld z=%ld  heading=%.1f graden\n",
			         (long)x, (long)y, (long)z, heading);
		  }
	  }
    /* USER CODE END WHILE */
