...
// *******************************************************************************************************************************************
// Vanaf dit punt zijn de functies zelf toegevoegd

/*
 * @brief zet de roll pitch en yaw struct op de standaardwaardes en stel de limieten ervan in
 */
void Init_RateLoops(void) {
    PID_HandleInit(&hPidRateRoll);
    PID_HandleInit(&hPidRatePitch);
    PID_HandleInit(&hPidRateYaw);

    PID_SetUpperOutputLimit(&hPidRateRoll, 1000);
    PID_SetLowerOutputLimit(&hPidRateRoll, -1000);
    PID_SetUpperOutputLimit(&hPidRatePitch, 1000);
    PID_SetLowerOutputLimit(&hPidRatePitch, -1000);
    PID_SetUpperOutputLimit(&hPidRateYaw, 1000);
    PID_SetLowerOutputLimit(&hPidRateYaw, -1000);
}

/*
 * @brief We gaan de output van de 3 PID controllers nemen en dit doorsturen voor de motoraansturing
 * @param De frame struct waar de data van de BMI330 instaat
 * @param 2-4 Dit zijn de gewenste waardes die je wilt bereiken
 */
void Run_RateLoop_Iteration(BMI330_Frame *current_frame, int16_t setpoint_roll, int16_t setpoint_pitch, int16_t setpoint_yaw) {

    int16_t out_roll  = PID_Controller(&hPidRateRoll,  setpoint_roll  - current_frame->gyr_x);
    int16_t out_pitch = PID_Controller(&hPidRatePitch, setpoint_pitch - current_frame->gyr_y);
    int16_t out_yaw   = PID_Controller(&hPidRateYaw,   setpoint_yaw   - current_frame->gyr_z);

    Apply_Motor_Mixing(out_roll, out_pitch, out_yaw);
}
/**
  * @}
  */
