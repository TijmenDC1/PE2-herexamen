...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  printf("\r\n--- STM32 IS OPGESTART ---\r\n");
      HAL_Delay(1000); // Wacht even 1 seconde
      printf("1 seconde voorbij. Start nu de GPS...\r\n");
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == UART7) {
        printf("\r\n[!] UART7 ERROR GEVONDEN: %lu\r\n", huart->ErrorCode);

        // Error code 8 = Overrun (ORE)
        // Error code 2 = Framing Error (verkeerde baudrate)

        // Start de interrupt geforceerd opnieuw op na een error:
        HAL_UARTEx_ReceiveToIdle_IT(&huart7, rx_temp_buffer, sizeof(rx_temp_buffer));
    }
}
/*
