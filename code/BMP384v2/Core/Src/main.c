...
/* USER CODE BEGIN PV */
static BMP384_Calib baro_calib;
...
int main(void)
{
  ...
  MX_UART8_Init();
  /* USER CODE BEGIN 2 */
  /* Barometer-test: init de nieuwe driver en meet de grondduk als referentie
   * voor de hoogteberekening. Bij een fout blijft de lus hieronder gewoon
   * elke keer opnieuw proberen te lezen (en falen), zodat je aan de UART8-log
   * ziet wat er mis is i.p.v. dat het stilletjes doorloopt. */
  BMP384_Status baro_status = BMP384_Init(&baro_calib);

  if (baro_status == BMP384_OK) {
      /* eenmalig de ruwe ADC-waarden tonen: als die 0 of 16777215 zijn,
       * meet de sensor niet en heeft compenseren geen zin */
      uint32_t rp = 0, rt = 0;
      HAL_Delay(50);
      if (BMP384_ReadRaw(&rp, &rt) == BMP384_OK) {
          printf("BMP384: ruwe waarden druk=%lu temp=%lu\n",
                 (unsigned long)rp, (unsigned long)rt);
      }

      if (rp == 0 && rt == 0) {
          BMP384_Diagnose();
          BMP384_ForcedTest(&rp, &rt);
          printf("BMP384: na forced-test druk=%lu temp=%lu\n",
                 (unsigned long)rp, (unsigned long)rt);
          BMP384_Diagnose();
      }

      float sum_pa = 0.0f;
      const int ground_samples = 20;
      int got = 0;
      int tries = 0;
      while (got < ground_samples && tries < 500) {
          tries++;
          if (BMP384_IsDataReady()) {
              float p;
              if (BMP384_ReadPressureTemperature(&baro_calib, &p, NULL) == BMP384_OK) {
                  sum_pa += p;
                  got++;
              }
          }
      ...
      if (got == ground_samples) {
          g_ground_pressure_pa = sum_pa / (float)ground_samples;
          printf("BMP384: grondduk = %.1f Pa\n", g_ground_pressure_pa);
      } else {
          printf("BMP384: geen geldige metingen (%d/%d), grondduk niet bepaald\n",
                 got, ground_samples);
          baro_status = BMP384_ERR_DATA;
      }
  }else{
	  printf("BMP is niet OKE\n");
  }
  /* USER CODE END 2 */
  ...
  {
	  if (baro_status != BMP384_OK) {
		  printf("BMP384: niet geinitialiseerd, geen metingen\n");
		  HAL_Delay(1000);
	  } else if (BMP384_IsDataReady()) {
		  float pressure_pa, temperature_c;
		  if (BMP384_ReadPressureTemperature(&baro_calib, &pressure_pa, &temperature_c) == BMP384_OK) {
			  float altitude_m = BMP384_PressureToAltitude(pressure_pa, g_ground_pressure_pa);
			  printf("BMP384: druk=%.1fPa  temp=%.2fC  hoogte=%.2fm\n", pressure_pa, temperature_c, altitude_m);
		  }
		  HAL_Delay(100);
	  }
    /* USER CODE END WHILE */
