...
// gyro-offset (in dps) die bij het opstarten wordt gemeten, zie MPU6050_Calibrate_Gyro
static float gyro_bias_x = 0.0f;
static float gyro_bias_y = 0.0f;
static float gyro_bias_z = 0.0f;

// uit DshotFCMetMPU6050/Core/Src/MPU6050.c
...
void MPU6050_Read_Gyro(float *gx, float *gy, float *gz) {
    ...
    // en meteen de bij het opstarten gemeten offset eraf trekken
    *gx = (float)raw_x / 65.5f - gyro_bias_x;
    *gy = (float)raw_y / 65.5f - gyro_bias_y;
    *gz = (float)raw_z / 65.5f - gyro_bias_z;
}
...
void MPU6050_Calibrate_Gyro(uint16_t samples) {
    ...
    // zorg dat we de ruwe waarden middelen (bias tijdelijk op 0)
    ...
    for (uint16_t i = 0; i < samples; i++) {
        ...
        sum_y += gy;
        sum_z += gz;
        HAL_Delay(1); // ~1 sample per ms (sample rate = 1kHz)
    }
