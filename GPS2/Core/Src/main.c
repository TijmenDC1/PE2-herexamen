...
*/
ublox_parser_state_t gps_state;
typedef struct {
    int32_t latitude;
    int32_t longitude;
    int32_t altitude;
    int32_t velocity_cm_s;
    uint8_t satellites;
    uint8_t fix_type;
    bool new_data_available;
} DroneGPS_Data_t;

volatile DroneGPS_Data_t current_gps_data;
...
void UBX_Set_Config(uint32_t keyID, uint8_t value) {
    uint8_t packet[17];
    packet[0] = 0xB5; // Header A
    packet[1] = 0x62; // Header B
    packet[2] = 0x06; // Class 0x06 (CFG)
    packet[3] = 0x8A; // ID 0x8A (VALSET)
    packet[4] = 0x09; // Payload Length LSB (9 bytes)
    packet[5] = 0x00; // Payload Length MSB
    packet[6] = 0x00; // Version
    packet[7] = 0x01; // Layers: 0x01 = Opslaan in RAM-geheugen
    packet[8] = 0x00; // Reserved
    packet[9] = 0x00; // Reserved

    // Key ID (Little Endian formaat)
    packet[10] = (uint8_t)(keyID & 0xFF);
    packet[11] = (uint8_t)((keyID >> 8) & 0xFF);
    packet[12] = (uint8_t)((keyID >> 16) & 0xFF);
    packet[13] = (uint8_t)((keyID >> 24) & 0xFF);

    // Waarde invullen
    packet[14] = value;

    // Checksum berekenen (over byte 2 tot en met 14)
    ...
    // Stuur via UART7 (Zorg dat &huart7 klopt met jouw setup)
    // We gebruiken hier de normale (blocking) Transmit omdat dit
    // alleen tijdens het opstarten gebeurt.
    HAL_UART_Transmit(&huart7, packet, 17, 100);

    // Korte pauze (20ms) zodat de GPS de aanpassing kan verwerken
    HAL_Delay(20);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  */
  ...
  /* USER CODE BEGIN 2 */
  //BMI330_Init();
  ublox_init(&gps_state);
  printf("GPS Configureren...\n");
  // 1. Vertel UART1 (van de GPS) dat hij UBX-protocol mag UITzenden
      // Key: CFG-UART1OUTPROT-UBX (0x10740001) -> Value: 1 (Aan)
      UBX_Set_Config(0x10740001, 1);

      // 2. Vertel UART1 dat hij NMEA-protocol moet STOPPEN met uitzenden
      // Key: CFG-UART1OUTPROT-NMEA (0x10740002) -> Value: 0 (Uit)
      UBX_Set_Config(0x10740002, 0);

      // 3. Zet het specifieke UBX-NAV-PVT bericht AAN op UART1
      // Key: CFG-MSGOUT-UBX_NAV_PVT_UART1 (0x20910007) -> Value: 1 (Aan)
      UBX_Set_Config(0x20910007, 1);
      printf("Klaar! Start inlezen...\r\n");
      HAL_UART_AbortReceive(&huart7);
          __HAL_UART_CLEAR_FLAG(&huart7, UART_CLEAR_OREF);

          // Start de interrupt, en sla het resultaat op om te checken of het lukt
          HAL_StatusTypeDef status = HAL_UARTEx_ReceiveToIdle_IT(&huart7, rx_temp_buffer, sizeof(rx_temp_buffer));

          if (status != HAL_OK) {
              printf("FOUT: Kon de UART interrupt niet starten!\r\n");
          }else{
        	  printf("Sensor werkt\n");
          }
  /* USER CODE END 2 */
  ...
  {
	  //printf("TEstEST\n");
	  // 3. Verwerk alle binnengekomen GPS bytes
	  	  while (UART_Has_Data()) {
	  	      uint8_t data = UART_Read_Byte();

	  	      if (ublox_process_byte(&gps_state, data)) {
	  	          // We hebben een compleet en geldig GPS bericht ontvangen!
	  	          // Print eerst de basisinformatie die áltijd doorkomt:
	  	          printf("Tijd: %02d:%02d:%02d | Sats: %d | Fix Type: %d\r\n",
	  	                 gps_state.pvtData.hour,
	  	                 gps_state.pvtData.min,
	  	                 gps_state.pvtData.sec,
	  	                 gps_state.pvtData.satellites,
	  	                 gps_state.pvtData.fix_type);

	  	          // Als we daadwerkelijk een locatie hebben (2D of 3D fix)
	  	          if (gps_state.pvtData.fix_type >= 2) {
	  	              // Let op: %ld wordt gebruikt voor int32_t (long decimal)
	  	              printf("--> Lat: %ld | Lon: %ld | Hoogte: %ld mm\r\n",
	  	                     gps_state.pvtData.latitude,
	  	                     gps_state.pvtData.longitude,
	  	                     gps_state.pvtData.altitude_msl);
	  	          }

	  	          // Reset flag
	  	          gps_state.newDataReady = false;
	  	      }

	  	  }
	  	if (gps_state.pvtData.fix_type >= 3) { // Gebruik in de lucht alleen een 3D fix!
	  	    current_gps_data.latitude = gps_state.pvtData.latitude;
	  	    current_gps_data.longitude = gps_state.pvtData.longitude;
	  	    current_gps_data.altitude = gps_state.pvtData.altitude_msl;
	  	    current_gps_data.velocity_cm_s = gps_state.pvtData.speed_2d / 10; // Omrekenen van mm/s naar cm/s
	  	    current_gps_data.satellites = gps_state.pvtData.satellites;
	  	    current_gps_data.fix_type = gps_state.pvtData.fix_type;
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size){
	if(huart->Instance == UART7){
		...
		// START DE LUIZERAAR OPNIEUW:
		// Vertel de HAL dat hij weer de temp_buffer mag gaan vullen
		HAL_UARTEx_ReceiveToIdle_IT(&huart7, rx_temp_buffer, sizeof(rx_temp_buffer));
	}
}
/*
